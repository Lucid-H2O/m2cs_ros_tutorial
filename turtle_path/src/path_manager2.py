...
def cb_walk(req):
    ...
    while ('''goal not reached'''): # control loop
        ...
        velocity = Twist()
        velocity.linear.x = req.distance
...
def cb_orientation(req):
    ...
    while (req.orientation != cur_pos.theta): # control loop
...
if __name__ == '__main__':
    ...
    sub = rospy.Subscriber('/turtle1/pose', Pose, queue_size = 1) # subscriber of the turtle position, callback to cb_pose
